...
{$ENDIF}
          ...
          if Fno_save then begin
             SendEKMessage;
          end;

          if not FNextSave then
...
begin
{$IFDEF DEBUG}
  lLog.save('t', 'TK marker | '+copy(Data, 28, 1)+' | Fno_save | '+booltostr(Fno_save)+' | FNextSave | '+booltostr(FNextSave));
{$ENDIF}
...
{$ENDIF}
    ...
    //{ test }      SqlSaveInBuffer(trim(copy(Data, 60, 6)))
            ...
            lLog.save('w', 'weight | '+trim(copy(Data, 40, 6)));
            FNextSave := false;
          end
          else
...
{$ENDIF}
     ...
     FNextSave := ComparePkdat;//разрешаем снятие следующего показания
end;
