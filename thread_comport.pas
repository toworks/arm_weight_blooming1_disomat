...
type
  ...
  private
    ...
    function CleanData(str: AnsiString): AnsiString;
  protected
...
//              lLog.save('i', 'main do NextSave | '+booltostr(NextSave)+' | '+booltostr(FNextSave));
...
begin
 ...
 { copy(Data, 2, 6);          //ответ по весу
  ...
  weight := CleanData( trim(copy(Data, 40, 6)) );
...
{$IFDEF DEBUG}
  lLog.save('t', 'step 3');
{$ENDIF}
      if ( weight <> '' ) then begin //не пропускать пустых значений
          if pkdat <> '' then begin
    //{ test }      SqlSaveInBuffer(trim(copy(Data, 60, 6)))
            SqlSaveInBuffer(weight);
            NextSave := false;
//            lLog.save('w', 'ReceiveTKData | '+trim(copy(Data, 40, 6)));
          end
          else
            lLog.save('w', 'заготовка не выбрана'+#9+'weight | '+weight);
      end;
  end;
end;
...
var
   weight: AnsiString;
begin
     weight := CleanData( trim(copy(FMessageData, 40, 6)) );
     if assigned(CalibrationForm.l_calibration) then
        if weight <>  '' then
          CalibrationForm.l_calibration.caption := weight;
end;
...
{ id_asutp состоит из полей pkdat+num+num_ingot, где в АСУТП pkdat состоит
  ...
  no_save := false;//разрешаем отправку подтверждения в контроллер
end;
...
function TThreadComPort.CleanData(str: AnsiString): AnsiString;
var
   buf: AnsiString;
   i: integer;
begin
